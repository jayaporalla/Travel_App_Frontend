...
export const Payment = () => {
        ...
        (async () => {
            try {
                const { data } = await axios.get(
                    `https://travel-app-backend-uy83.onrender.com/api/hotels/${id}`
                );
                setSingleHotel(data);
            } catch(err){
                console.log(err);
    ...
    const handleConfirmBookingClick = async () => {
        ...
        const options = {
            key: "rzp_test_hwWBX0C5m9P8ll",
            amount: totalPayableAmount * 100,
            currency: "INR",
            name: "TravelO",
            email: "jayasri@gmail.com",
            contact: "9632587410",
            ...
            handler: ({ payment_id }) => {
                ...
                checkOutDate: checkInDate.toLocaleDateString("en-US", { day: "numeric", month: "short"}),
                totalPayableAmount});
                navigate("/order-summary");
            },
            prefill: {
                name: "Jayasri Poralla",
                email: "jayasri@gmail.com",
                contact: "9632587410"
            },
        };
